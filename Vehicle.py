...
class Vehicle:
    ...
    def drawVehicle(self, frame):
        area = self.stat[cv2.CC_STAT_AREA]
        ...
        cv2.putText(
            frame, 
            self.classify(area, self.stat[cv2.CC_STAT_WIDTH], self.stat[cv2.CC_STAT_HEIGHT])+' '+str(self.vid),
            initial_point_text, 
            cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), 1
        )

    def classify(self, area, width, height):
